		...
		if( IntersectRect(&intersect,&tmp, &cull) == TRUE )
		{
			if( m_vTiles[i].m_Layer > 1)
			{
				...
				if( tmp->IsOn() &&((tmp->GetLightType() > 1 ) || m_vTiles[i].shadow))
				{
					...
					pView->DrawStaticTexture(m_vTiles[i].m_nTileID, (int)m_vTiles[i].m_nWorldPosX-cam.x, (int)m_vTiles[i].m_nWorldPosY-cam.y, 1.0f, 1.25f, &m_vTiles[i].m_rImageRect, 32, 120, angle, D3DCOLOR_ARGB( 200, 0, 0, 0));
				}
