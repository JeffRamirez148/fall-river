	...
	if( GamePlayState::GetInstance()->GetPlayer()->GetWeaponType() != WPN_MACHETE)
	{
		...
		if( m_bShot == true )
		{
			...
			if(m_aClipAnim.fTime >= thisAnim->frames[m_aClipAnim.curAnimation][m_aClipAnim.curFrame].duration-0.1f)
			{
				...
				if(((unsigned int)m_aClipAnim.curFrame == thisAnim->frames[m_aClipAnim.curAnimation].size()) && thisAnim->looping[m_aClipAnim.curAnimation])
					m_aClipAnim.curFrame = 0;
				else if((unsigned int)m_aClipAnim.curFrame == thisAnim->frames[m_aClipAnim.curAnimation].size() && !thisAnim->looping[m_aClipAnim.curAnimation])
				{
					m_aClipAnim.curFrame = 0;
