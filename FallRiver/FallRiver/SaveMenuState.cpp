...
bool SaveMenuState::Input()
{
	...
	if( ( (m_pDI->KeyPressed(DIK_RIGHT) || m_pDI->KeyPressed(DIK_D) ) && m_bCheck) || (m_pDI->JoystickGetLStickDirPressed(DIR_RIGHT,0) && m_bCheck) )
	{
		m_nCursPosX += 100;
		if( m_nCursPosX > 380 )
			m_nCursPosX = 280;
	...
	else if( ( (m_pDI->KeyPressed(DIK_LEFT) || m_pDI->KeyPressed(DIK_A) ) && m_bCheck) || (m_pDI->JoystickGetLStickDirPressed(DIR_RIGHT,0) && m_bCheck) )
	{
		m_nCursPosX -= 100;
		if( m_nCursPosX < 280 )
			m_nCursPosX = 380;
